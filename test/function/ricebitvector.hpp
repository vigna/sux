...
static RiceBitVector<AT> *build_bitvector(const vector<uint64_t> &keys, const int golomb_param) {
	RiceBitVector<AT> *r = new RiceBitVector<AT>();
...
template <sux::util::AllocType AT = sux::util::AllocType::MALLOC>
static void test_rice_trees(RiceBitVector<AT> *r, const vector<uint64_t> &keys, const int golomb_param, const size_t tree_offset) {
	for (size_t t = 0; t < rice_test_ntrees; ++t) {
