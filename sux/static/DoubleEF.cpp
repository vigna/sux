...
DoubleEF::DoubleEF(const std::vector<uint64_t>& cum_keys, const std::vector<uint64_t>& position) {
	...
	for(size_t i = 1; i <= num_buckets; ++i) {
		const auto nkeys_delta = cum_keys[i] - cum_keys[i - 1];
...
void DoubleEF::get(const uint64_t i, uint64_t& cum_keys, uint64_t& cum_keys_next, uint64_t& position) {
	...
	const uint64_t select_cum_keys = select64(delta_cum_keys, window_cum_keys);
	...
	position = ((curr_word_position * 64 + select64(delta_position, window_position) - i) << l_position | lower & lower_bits_mask_position) + bit_delta + int64_t(bits_per_key_fixed_point * cum_keys >> 20);
...
void DoubleEF::load(FILE* fp) {
	size_t nbytes;
	nbytes = fread(&num_buckets, sizeof(num_buckets), (size_t)1, fp);
	nbytes = fread(&u_cum_keys, sizeof(u_cum_keys), (size_t)1, fp);
	nbytes = fread(&u_position, sizeof(u_position), (size_t)1, fp);
	nbytes = fread(&cum_keys_min_delta, sizeof(cum_keys_min_delta), (size_t)1, fp);
	nbytes = fread(&min_diff, sizeof(min_diff), (size_t)1, fp);
	nbytes = fread(&bits_per_key_fixed_point, sizeof(bits_per_key_fixed_point), (size_t)1, fp);
	...
	const uint64_t words_lower_bits = lower_bits_size_words();
	lower_bits = new uint64_t[words_lower_bits];
	nbytes = fread(lower_bits, sizeof(uint64_t), (size_t)words_lower_bits, fp);
	const uint64_t words_cum_keys = cum_keys_size_words();
	upper_bits_cum_keys = new uint64_t[words_cum_keys]();
	nbytes = fread(upper_bits_cum_keys, sizeof(uint64_t), (size_t)words_cum_keys, fp);
	const uint64_t words_position = position_size_words();
	upper_bits_position = new uint64_t[words_position]();
	nbytes = fread(upper_bits_position, sizeof(uint64_t), (size_t)words_position, fp);

	const uint64_t jump_words = jump_size_words();
	jump = new uint64_t[jump_words];
	nbytes = fread(jump, sizeof(uint64_t), (size_t)jump_words, fp);
}
