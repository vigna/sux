...
private:
	...
	__inline static int find_far_close( const uint64_t word, int k ) {
		...
		const uint64_t open4mask = ( ( ( ( open4 & MSBS_STEP_4 ) >> 3 ) | MSBS_STEP_4 ) - ONES_STEP_4 ) ^ MSBS_STEP_4;
		
		uint64_t closed4 = ( ( closed4eccess | MSBS_STEP_4 ) - open4eccess ) ^ MSBS_STEP_4;
		const uint64_t closed4mask = ( ( ( ( closed4 & MSBS_STEP_4 ) >> 3 ) | MSBS_STEP_4 ) - ONES_STEP_4 ) ^ MSBS_STEP_4;
		...
		const uint64_t open8mask =  ( ( ( ( open8 & MSBS_STEP_8 ) >> 7 ) | MSBS_STEP_8 ) - ONES_STEP_8 ) ^ MSBS_STEP_8;

		uint64_t closed8 = ( ( closed8eccess | MSBS_STEP_8 ) - open8eccess ) ^ MSBS_STEP_8;
		const uint64_t closed8mask = ( ( ( ( closed8 & MSBS_STEP_8 ) >> 7 ) | MSBS_STEP_8 ) - ONES_STEP_8 ) ^ MSBS_STEP_8;
		...
		const uint64_t open16mask =  ( ( ( ( open16 & MSBS_STEP_16 ) >> 15 ) | MSBS_STEP_16 ) - ONES_STEP_16 ) ^ MSBS_STEP_16;

		uint64_t closed16 = ( ( closed16eccess | MSBS_STEP_16 ) - open16eccess ) ^ MSBS_STEP_16;
		const uint64_t closed16mask = ( ( ( ( closed16 & MSBS_STEP_16 ) >> 15 ) | MSBS_STEP_16 ) - ONES_STEP_16 ) ^ MSBS_STEP_16;
		...
		const uint64_t open32mask =  ( ( ( ( open32 & MSBS_STEP_32 ) >> 31 ) | MSBS_STEP_32 ) - ONES_STEP_32 ) ^ MSBS_STEP_32;

		uint64_t closed32 = ( ( closed32eccess | MSBS_STEP_32 ) - open32eccess ) ^ MSBS_STEP_32;
		const uint64_t closed32mask = ( ( ( ( closed32 & MSBS_STEP_32 ) >> 31 ) | MSBS_STEP_32 ) - ONES_STEP_32 ) ^ MSBS_STEP_32;
...
#define L (0x3830282018100800ULL+0x0202020202020202ULL)
	__inline static int find_near_close( uint64_t word ) {
		...
		byte_sums = ( ( byte_sums + ( byte_sums >> 4 ) ) & 0x0f * ONES_STEP_8 ) * ( ONES_STEP_8 << 1 ); // Twice the number of open parentheses (cumulative by byte)

		byte_sums = ( ( MSBS_STEP_8 | byte_sums ) - L ) ^ MSBS_STEP_8; // Closed excess per byte

		update = ( ~( ( ( byte_sums - ONES_STEP_8 ) & MSBS_STEP_8 ) >> 7 ) & ONES_STEP_8 ) - ONES_STEP_8;
		zeroes = ( MSBS_STEP_8 | ONES_STEP_8 * 1 ) & update;
		
		word >>= 2;
		byte_sums -= ONES_STEP_8 * 2 - ( ( word & ( ONES_STEP_8 << 1 ) ) + ( word << 1 & ( ONES_STEP_8 << 1 ) ) );
		update = ( ~( ( ( ( ( byte_sums | zeroes ) - ONES_STEP_8 ) ^( byte_sums | zeroes ) ) ) >> 7 ) & ONES_STEP_8 ) - ONES_STEP_8;
...
#define L (0x4038302820181008ULL)
	__inline static int find_near_close( const uint64_t word ) {
		...
		update = ( ( ( ( byte_sums | MSBS_STEP_8 ) - ONES_STEP_8 ) >> 7 & ONES_STEP_8 ) | MSBS_STEP_8 ) - ONES_STEP_8;
		zeroes = ( MSBS_STEP_8 >> 1 | ONES_STEP_8 * 7 ) & update;
		
		byte_sums -= ONES_STEP_8 * 2 - ( ( word >> 6 & ( ONES_STEP_8 << 1 ) ) + ( word >> 5 & ( ONES_STEP_8 << 1 ) ) );
		update = ( ( ( ( byte_sums | MSBS_STEP_8 ) - ONES_STEP_8 ) >> 7 & ONES_STEP_8 ) | MSBS_STEP_8 ) - ONES_STEP_8;
		zeroes = zeroes & ~update | ( MSBS_STEP_8 >> 1 | ONES_STEP_8 * 5 ) & update;
		
		byte_sums -= ONES_STEP_8 * 2 - ( ( word >> 4 & ( ONES_STEP_8 << 1 ) ) + ( word >> 3 & ( ONES_STEP_8 << 1 ) ) );
		update = ( ( ( ( byte_sums | MSBS_STEP_8 ) - ONES_STEP_8 ) >> 7 & ONES_STEP_8 ) | MSBS_STEP_8 ) - ONES_STEP_8;
		zeroes = zeroes & ~update | ( MSBS_STEP_8 >> 1 | ONES_STEP_8 * 3 ) & update;
		
		byte_sums -= ONES_STEP_8 * 2 - ( ( word >> 2 & ( ONES_STEP_8 << 1 ) ) + ( word >> 1 & ( ONES_STEP_8 << 1 ) ) );
		update = ( ( ( ( byte_sums | MSBS_STEP_8 ) - ONES_STEP_8 ) >> 7 & ONES_STEP_8 ) | MSBS_STEP_8 ) - ONES_STEP_8;
	...
	__inline static int find_far_close( uint64_t word, int k ) {
		...
		for( int i = 0; i < 64; i += 8 ) {
			...
			if ( k < 8 && ( f = find_far_close8[ b ][ k ] ) != 8 ) return i + f;
			k += delta_far_close[ b ]; 
			word >>= 8;
		}
	...
	__inline static int find_near_close( uint64_t word ) {
		...
		for( int i = 0; i < 64; i += 8 ) {
			const int b = word & 0xFF;
			if ( e <= 4 && ( f = find_near_close8[ b ][ e ] ) != 8 ) return i + f;
