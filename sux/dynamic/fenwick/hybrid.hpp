...
template <template <size_t> class TOP, template <size_t> class BOTTOM, size_t BOUND, size_t CUT> class Hybrid : public FenwickTree {
  private:
  ...
  public:
	Hybrid(uint64_t sequence[], size_t size) : Size(size), TopFenwick(buildTop(sequence, size)) {
		...
		for (size_t i = 0; i < TopFenwick.size(); i++) TopFenwick.add(i + 1, BottomForest[i].prefix(BottomForest[i].size()));
	}
	...
	virtual size_t bitCount() const {
		size_t bottomSize = 0;
		for (auto &t : BottomForest) bottomSize += t.bitCount();
  ...
  private:
	TOP<TOPBOUND> buildTop(const uint64_t sequence[], size_t size) const {
		...
		for (size_t i = 1; i <= topsize; i++) subseq[i - 1] = sequence[i * (BottomSize + 1) - 1];
	...
	friend std::ostream &operator<<(std::ostream &os, const Hybrid<TOP, BOTTOM, BOUND, CUT> &ft) {
		...
		os << ft.TopFenwick;
		for (size_t i = 0; i < nbottom; ++i) os << ft.BottomForest[i];
	...
	friend std::istream &operator>>(std::istream &is, Hybrid<TOP, BOTTOM, BOUND, CUT> &ft) {
		...
		for (size_t i = 0; i < nbottom; ++i) ft.BottomForest.push_back(readNextBottom());
